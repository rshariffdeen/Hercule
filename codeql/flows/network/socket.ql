...
module MyFlowConfiguration implements DataFlow::ConfigSig {
  ...
  predicate isSink(DataFlow::Node sink) { 
    ...
    sink.(DataFlow::CallCfgNode).getFunction().toString().regexpMatch(".*(write|sendall|connect|send|post|put|patch|delete|get|exec|eval|dumps?).*") or
    sink.(DataFlow::MethodCallNode).getMethodName()      .regexpMatch(".*(write|sendall|connect|send|post|put|patch|delete|get|exec|eval|dumps?).*"))
